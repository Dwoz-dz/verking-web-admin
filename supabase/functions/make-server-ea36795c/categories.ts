  ...
  try {
    ...
    if (await useDB()) {
      try {
        ...
        // Let Postgres generate the real UUID — our `uid()` produces non-UUID strings
        // which the `uuid` column rejects with `invalid input syntax`.
        const insertData: Record<string, any> = { ...base };
...
export async function deleteCategory(c: any) {
  try {
    ...
    if (await useDB()) {
      try {
        // FK safety: check if any products still reference this category.
          ...
          return c.json(
            {
              ...
              message: `${productCount} produit(s) utilisent encore cette catégorie.`,
        ...
        // Reassign or orphan products before deletion.
        if (productCount > 0) {
