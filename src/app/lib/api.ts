...
export const api = {
  ...
  async post(path: string, body: any) {
    const res = await fetch(`${API_BASE}${path}`, { method: 'POST', headers: publicHeaders(), body: JSON.stringify(body) });
...
export const adminApi = {
  ...
  async post(path: string, body: any, token: string) {
    ...
    const res = await fetch(`${API_BASE}${path}`, { method: 'POST', headers: adminHeaders(token), body: JSON.stringify(body) });
    if (!res.ok) {
  ...
  async put(path: string, body: any, token: string) {
    ...
    const res = await fetch(`${API_BASE}${path}`, { method: 'PUT', headers: adminHeaders(token), body: JSON.stringify(body) });
    if (!res.ok) {
        if (res.status === 401) window.dispatchEvent(new Event('vk_admin_logout'));
  ...
  async del(path: string, token: string) {
    ...
    const res = await fetch(`${API_BASE}${path}`, { method: 'DELETE', headers: adminHeaders(token) });
    if (!res.ok) {
        if (res.status === 401) window.dispatchEvent(new Event('vk_admin_logout'));
